...
module.exports = {
  ...
  getStackSummary(stackName) {
    ...
    function listStackResources(provider, StackName, NextToken) {
      ...
      function request() {
        if (provider.getCloudFormationClient) {
          return provider.getCloudFormationClient()
            .then(client => client.send(new ListStackResourcesCommand(params)));
        }

        if (provider.getAwsSdkV3Config) {
          splitStacks.cloudFormationClientPromise = splitStacks.cloudFormationClientPromise || provider
            .getAwsSdkV3Config()
            .then(config => new CloudFormationClient(config));

          return splitStacks.cloudFormationClientPromise
            .then(client => client.send(new ListStackResourcesCommand(params)));
        }

        if (provider.request) {
          return provider.request('CloudFormation', 'listStackResources', params);
        }

        return Promise.reject(new Error('AWS provider does not expose request(), getCloudFormationClient(), or getAwsSdkV3Config()'));
      }
